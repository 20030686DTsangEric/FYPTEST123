...
if (isset($_POST["admin_email_address"])) {
    ...
    if ($total_row == 0) {
        $object->query = "
			...
			WHERE teacher_email_address = :admin_email_address
		";
        $object->execute($data);
        ...
        if ($object->row_count() == 0) {
            ...
            foreach ($result as $row) {
                if ($row["teacher_status"] == 'Inactive') {
                    $error = '<div class="alert alert-danger">Your account is Inactive, Contact Admin</div>';
                } else {
                    if ($_POST["admin_password"] == $row["teacher_password"]) {
                        $_SESSION['admin_id'] = $row['teacher_id'];
                        $_SESSION['type'] = 'Teacher';
                        $url = $object->base_url . 'admin/teacher_schedule.php';
                    } else {
